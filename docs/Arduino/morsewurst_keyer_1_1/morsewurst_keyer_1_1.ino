...
void telemetryHello();
void telemetryKeyRaw(const char* source, const char* state, uint64_t t);
void telemetryKeyElement(const char* source, char element, const char* state, uint64_t t);
float estimatedWpm();
...
void telemetryHello() {
  ...
  Serial.print("{\"v\":1,\"type\":\"hello\",\"app\":\"morsewurst\",\"device\":\"Morsewurst\",\"fw\":\"1.1\",\"mode\":\"raw_timing\"}");
  Serial.println();
}
...
void telemetryHeartbeat() {
  ...
  Serial.print("{\"v\":1,\"type\":\"heartbeat\",\"app\":\"morsewurst\",\"device\":\"Morsewurst\",\"fw\":\"1.1\",\"mode\":\"raw_timing\",\"uptime\":");
...
// ======================================================
// V1 key down/up telemetry
// ======================================================
void telemetryKeyRaw(const char* source, const char* state, uint64_t t) {
  if (!settings.serialTelemetry) return;

  Serial.print("{\"v\":1,\"type\":\"key\",\"src\":\"");
  Serial.print(source);
  Serial.print("\",\"state\":\"");
  Serial.print(state);
  Serial.print("\",\"t\":");
...
void handleStraightKey(bool straightPressed) {
  if (!settings.straightEnabled) {
    if (lastStraightState) {
      telemetryKeyRaw("straight", "up", nowTime());
    }
    audioStop();
  ...
  if (straightPressed && !lastStraightState) {
    ...
    audioStart();
    telemetryKeyRaw("straight", "down", straightDownStart);
  }

  if (!straightPressed && lastStraightState) {
    uint64_t endTime = nowTime();
    audioStop();
    telemetryKeyRaw("straight", "up", endTime);
...
void toneWithIambicSampling(uint64_t duration, char currentElement) {
  ...
  audioStart();
  telemetryKeyElement("iambic", currentElement, "down", start);
  ...
  uint64_t endTime = nowTime();
  audioStop();
  telemetryKeyElement("iambic", currentElement, "up", endTime);
}
...
void setup() {
  ...
  u8g2.begin();
  drawMessage("Morsewurst 1.1", "Ladataan...");
