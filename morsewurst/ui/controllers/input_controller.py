...
class InputController:
    ...
    def is_keyboard_morse_key_event_dict(self, event: Dict[str, Any]) -> bool:
        """Return True when a V1 key event was generated by the computer keyboard."""
        return (
            event.get("type") == "key"
    ...
    def is_keyboard_morse_tone_event(self, event: Dict[str, Any]) -> bool:
        """Return True when an internally derived keyboard tone is being handled."""
        return (
            event.get("type") == "tone"
            and event.get("device") == "keyboard"
            and event.get("mode") == "keyboard_straight"
    ...
    def make_keyboard_morse_key_event(
        self,
        *,
        state: str,
        t_us: int,
        ...
        """Build a normal V1 straight-key down/up event for the virtual key."""
        return {
            "v": 1,
            "type": "key",
            "src": "straight",
    ...
    def handle_keyboard_morse_key_press(self, event: tk.Event) -> bool:
        ...
        self.handle_serial_event(
            self.make_keyboard_morse_key_event(
                state="down",
                t_us=app.keyboard_morse_press_t0_us,
                key=app.keyboard_morse_press_key,
            )
        )
    ...
    def key_event_key(self, event: Dict[str, Any]) -> Optional[tuple[str, str, str, str, str, str, int]]:
        """Return the deduplication key for one V1 key event."""
        try:
            clean = normalize_key_event(event)
            identity = key_event_identity(clean)
            return identity + (str(clean.get("state") or ""), int(clean["t"]))
        except Exception:
            return None

    def accept_key_event(self, event: Dict[str, Any]) -> bool:
        """Return True when the V1 key event is valid and not a duplicate."""
        app = self.app
        key = self.key_event_key(event)
        if key is None:
            return False
        if key == getattr(app, "last_key_event_key", None):
            return False
        app.last_key_event_key = key
        return True

    def record_key_event(self, event: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Append a V1 key event and derive a tone when the matching up arrives."""
        app = self.app
        now = time.time()
        ...
        clean = normalize_key_event(event)
        app.round.events.append(clean)

        active = getattr(app, "active_v1_key_events", None)
        if active is None:
            active = {}
            app.active_v1_key_events = active

        identity = key_event_identity(clean)
        state = str(clean.get("state") or "")

        if state == "down":
            active[identity] = clean
            return None

        down_event = active.pop(identity, None)
        ...
        if self.is_keyboard_morse_tone_event(tone):
            min_tone_us = int(getattr(config, "KEYBOARD_MORSE_MIN_TONE_US", 1_000))
            if duration_us < min_tone_us:
                return None
        elif (
            tone.get("src") == "straight"
            and tone.get("_derived_from") == "v1_key_down_up"
        ):
            min_tone_us = int(getattr(config, "SERIAL_STRAIGHT_KEY_MIN_TONE_US", 10_000))
            if duration_us < min_tone_us:
                log_app_event(
                    "app.input.v1_straight_key_bounce_ignored",
                    message="Very short physical straight-key V1 pulse was kept as raw key telemetry but ignored as a derived decoder tone.",
                    context={
                        "duration_us": duration_us,
                        "min_tone_us": min_tone_us,
                        "src": tone.get("src"),
                        "derived_from": tone.get("_derived_from"),
                    },
                )
                return None

        self.record_tone_event(tone)
        return tone

    def handle_key_event(self, event: Dict[str, Any]) -> None:
        """Handle one V1 key down/up event as the primary live input stream."""
        app = self.app
        ...
        try:
            ...
            log_app_event(
                "app.input.invalid_v1_key_event",
                level="warning",
                message="Invalid V1 key event was ignored.",
                context={"error": str(exc), "event_type": event.get("type")},
            )
            return
        ...
        self.maybe_start_practice_from_key(clean)

        if not app.round.accepting_input or app.round.finished:
            return

        tone = self.record_key_event(clean)

        if clean.get("state") == "down":
            app.practice_controller.start_round_clock_from_tone_event(clean)

        if tone is not None:
            app.practice_controller.start_round_clock_from_tone_event(tone)
    ...
    def handle_serial_event(self, event: Dict[str, Any]) -> None:
        ...
        if event_type == "tone" and event.get("_derived_from") != "v1_key_down_up":
            log_app_event(
                "app.input.completed_tone_ignored",
                level="warning",
                message="Completed tone event was ignored because V1 key down/up is the normal input stream.",
                context={"src": event.get("src"), "t0": event.get("t0"), "t1": event.get("t1")},
            )
            return
    ...
    def maybe_start_practice_from_key(self, event: Dict[str, Any]) -> None:
        ...
        if event.get("type") not in {"key", "tone"}:
            return
